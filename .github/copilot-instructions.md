...
- **Target/abi:** Defaults to `i686-unknown-linux-gnu` via [.cargo/config.toml](.cargo/config.toml#L1-L9) but the build picks up `TARGET` from Cargo for other triples. Keep `no_std`; avoid host syscalls.
...
- **Prereqs:** Host needs `grub-mkrescue` (and deps like `xorriso`) installed; ensure whichever Rust target you build for is added (e.g., `rustup target add i686-unknown-linux-gnu`).
